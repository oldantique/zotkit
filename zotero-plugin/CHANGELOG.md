...
- ~~新增问答自动同步到 Zotero 笔记：为当前条目维护一篇打有 `zotkit-chat` 标签的笔记，按会话分节追加最新问答，安全转换为笔记可用的 HTML 并与已有分节匹配合并，而非重复创建。~~ 已移除，见下方"移除"条目——该功能未随任何版本发布过。
...
- 新增阅读留痕（Paper Trail）：一个选区锚定问题的首轮回答完成后，自动在该选区位置写入一条 Zotero 高亮批注（专用色 `#a28ae5`，打上 `zotkit-chat` + `zotkit-open` 标签，comment 为问题原文 + 2–3 句答案要点），把"问过的位置"变成可跨设备看到的痕迹。首次触发写入前弹一次性授权卡（偏好 `extensions.zotkit.paperTrail`，默认 `unset`）；拒绝后本次会话只在本地记录锚点、不再写批注，也不会重复询问。每个问题最多写入一次批注，写入经同一条串行队列合批，回合含错误 entry 时不写批注、不落盘锚点。浮窗新增轻量确认 chip（"已留痕 · 第 N 页 · 撤销"），撤销会删除该批注并清除本地锚点，不留残留；浮窗"已理解"按钮把锚点标记为 resolved，仅做标签替换（`zotkit-open` → `zotkit-resolved`），不改动批注的内容、颜色或位置。
- 侧边栏新增"问题清单"：按页码列出当前论文的全部留痕锚点（● 未解决 / ✓ 已解决），点击可跳转到对应批注位置并展开该段对话。Reader 自带的批注侧栏为带 `zotkit-chat` 标签的高亮追加一个"继续对话"按钮，点击后打开（或按需切换到）浮动提问窗并定位到该锚点所在的对话线程。
