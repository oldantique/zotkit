...
- **选区芯片**:呼出时若 PDF 存在选中文本,浮窗顶部显示芯片(`已选 N 字 · 第 X 页`),该选区作为上下文自动附带——复用侧边栏 ⌘L 的既有机制(interaction-context 固定选区,12k 字符上限;`buildSelectionPrompt`/32k 是终端专用路径,不用于 chat)。芯片带 ✕ 可移除。无选区时不显示芯片,仅携带当前论文上下文。
