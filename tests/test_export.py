...
from zotkit import __version__, export
...
def test_two_good_keys_concatenated_in_input_order_with_new_cite_keys(monkeypatch):
    ...
    assert failed == []
    # provenance header: fixed line, real version interpolated, then a blank line
    assert text.splitlines()[0] == (
        f"% Generated by zotkit {__version__} -- do not hand-edit. "
...
def test_non_404_error_status_also_fails_the_key(monkeypatch, capsys):
    ...
    assert failed == ["AAAA1111"] and text == export.HEADER + "\n"
...
def test_all_keys_failing_yields_header_only_text(monkeypatch):
    monkeypatch.setattr(httpx, "get", _fake_get({}))
    text, failed = export.export_bibtex(_zot(), ["X1", "X2"])
    assert text == export.HEADER + "\n" and failed == ["X1", "X2"]
    assert "@" not in text
